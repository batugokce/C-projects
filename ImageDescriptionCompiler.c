...
Boolean saveImage ( Image image , char * fileName )
{
	...
	while ( row < image.height ){
		...
		while ( col < image.width ) {
			...
			fprintf( filex , " %d %d %d ", 	image.pixels[i].red,
...
{	
	...
	double angleDif = (endAngleDegrees - startAngleDegrees) / numberOfSegments ;

	while ( count < numberOfSegments){

		Point first  = makePoint(center.row-cos(angle*PI/180)*radius,
					center.col+sin(angle*PI/180)*radius );

		Point second = makePoint(center.row-cos((angle+angleDif)*PI/180)*radius,
					 center.col+sin((angle+angleDif)*PI/180)*radius );

		drawLine( image , first , second , color );
...
Boolean drawRectangle ( Image image , Point topLeft , unsigned short int width , unsigned short int height , Color color ){
	...
	Point topRight = { topLeft.row         , topLeft.col+width };
	Point botLeft  = { topLeft.row+height  , topLeft.col       };
	Point botRight = { topLeft.row+height  , topLeft.col+width };

	drawLine( image , topLeft  , topRight , color );
	drawLine( image , topLeft  , botLeft  , color );
	drawLine( image , botRight , topRight , color );
...
{
	double incCol,incRow;
	int count = 1, roww, coll;
	Point second;
	double increase = 360 / (double) numberOfSides;
	double dist = sqrt(2*radius*radius*(1 - cos((360/numberOfSides) * (PI / 180))));

	Point first  = makePoint( center.row-radius , center.col );

	double outer = 180 /(double) numberOfSides;
	incCol = cos(outer*PI/180) * dist;
	incRow = sin(outer*PI/180) * dist;
	coll = (ceil(incCol)-incCol < 0.5) ? ceil(incCol) : floor(incCol);
	roww = (ceil(incRow)-incRow < 0.5) ? ceil(incRow) : floor(incRow);

	second = makePoint( first.row + roww, first.col + coll );
...
{
	if(c!=13){
		int number = 128, i = 12, j=0;
		while(i >= 0){
			number = 128;
			j=0;
			while (j<8){
				if (number & characterRasters[c-32][i]){
